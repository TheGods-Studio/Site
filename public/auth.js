...
(function () {
  ...
  async function getMe() {
    try {
      ...
      return { user: j.user, roles: j.roles || [], permissions: j.permissions || [] };
    } catch (_) {
      return null;
    }
  }
  ...
  function applyIndexState(data) {
    const user = data && data.user ? data.user : null;
    const roles = data && data.roles ? data.roles : [];
    const permissions = data && data.permissions ? data.permissions : [];
    const isAdmin = roles.includes('Admin Master') || roles.includes('Admin') || roles.includes('Funcionario') || permissions.includes('dashboard.view');
    ...
    const adminLink = document.querySelector('.sidebar a[data-admin-link]');
    if (adminLink) adminLink.style.display = isAdmin ? '' : 'none';

    const settingsBtn = document.getElementById('settings-btn');
    if (settingsBtn) settingsBtn.hidden = !user;

    const adminPage = document.querySelector('[data-admin-only-page]');
    if (adminPage) {
      if (isAdmin) {
